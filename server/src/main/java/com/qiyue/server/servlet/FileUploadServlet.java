...
import com.qiyue.server.util.AESUtil;
import com.qiyue.server.util.FileUtil;
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (contentType != null && contentType.startsWith("multipart/")) {
            ...
            if (parts.size() == 1) {
                ...
                fileModel.setName(originName);
                fileModel.setType(type);
                fileModel.setSize(FileUtil.parseSize(part.getSize()));
                fileModel.setTime(new Timestamp(System.currentTimeMillis()));
                fileModel.setLocation(position);
                OutputStream out = response.getOutputStream();
                try {
                    // 对随机AES密钥进行RSA加密
                    fileModel.setEnvelope(RSAUtil.encryptByPubKey(AES_KEY, Static.PUBLIC_KEY));
                    // 将文件信息写入数据库
