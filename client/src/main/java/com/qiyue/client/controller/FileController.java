package com.qiyue.client.controller;

import com.alibaba.fastjson.JSON;
import com.qiyue.client.model.FileModel;
import com.qiyue.client.util.AESUtil;
import com.qiyue.client.util.FileUtil;
import com.qiyue.client.util.RSAUtil;
import com.qiyue.client.util.Static;
...
public class FileController {
    ...
    private HttpHeaders builderHeader() throws Exception {
        ...
        String signature = RSAUtil.encryptByPriKey(sid, Static.PRIVATE_KEY);
    ...
    public String index(Model model) throws Exception {
        ...
        try {
            ...
            if (result.getStatusCode() == HttpStatus.OK) {
                fileModelList = JSON.parseArray(result.getBody(), FileModel.class);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void index(@RequestParam("id") String id, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (result.getStatusCode() == HttpStatus.OK) {
                fileModel = JSON.parseObject(result.getBody(), FileModel.class);
            }
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("调用Server接口失败");
        ...
        if (fileModel != null) {
            ...
            if (result.getStatusCode() == HttpStatus.OK) {
                ...
                response.setHeader("content-disposition", "attachment;filename=" + URLEncoder.encode(fileModel.getName(), "UTF-8"));
                OutputStream out = response.getOutputStream();

                // 3.解密，输出到浏览器
                String key = RSAUtil.decryptByPriKey(fileModel.getEnvelope(), Static.PRIVATE_KEY);
